...
test_that("aic matches original aic.wge", {
  ...
  new1 <- aic(x1, p = 0:3, q = 0:2, type = "aic")
  ...
  new2 <- aic(x1, p = 0:3, q = 0:2, type = "bic")
  ...
  new3 <- aic(x1, p = 0:3, q = 0:2, type = "aicc")
  ...
  new4 <- aic(x2, p = 0:4, q = 0:2, type = "aic")
...
test_that("aic validates inputs", {

  x <- rnorm(100)

  expect_error(aic("not numeric", p = 0:3, q = 0:2), "`x` must be")
  expect_error(aic(numeric(0), p = 0:3, q = 0:2), "`x` must be")
  expect_error(aic(x, p = -1:3, q = 0:2), "`p` must be")
  expect_error(aic(x, p = 0:3, q = -1:2), "`q` must be")
})


test_that("aic returns comparison table", {

  result <- aic(rnorm(100), p = 0:2, q = 0:1)
...
test_that("aic has correct structure", {
...
test_that("aic print method works", {
...
test_that("aic handles failed models gracefully", {

  # Very short series where some models may fail
  x <- rnorm(30)
  expect_no_error(aic(x, p = 0:2, q = 0:2))
})
